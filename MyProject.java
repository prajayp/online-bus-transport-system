...
class MyProject extends User {
	...
	public void vbs() {
		...
		ImageIcon i = new ImageIcon("downloadreal.png");
		...
		JLabel labelt = new JLabel( i);

		labelt.setBounds(300, 35, 700, 150);
		label1.setBounds(600, 8, 700, 300);
		label1.setForeground(Color.RED);
		labelt.setForeground(Color.BLACK);
		JLabel label2 = new JLabel("BOOK YOUR TICKETS NOW !!");
		label2.setBounds(527, 200, 500, 40);

		JLabel label4 = new JLabel("Plans changed,Cancel your ticket easily");
		label4.setBounds(485, 340, 600, 40);
...
class BookNow extends User {
	...
	public static void bookframe() {
		...
		b1.setFont(font);
		b2.setFont(font);
		String[] source = { "KOLLAM", "KAYANKULAM", "KARUNAGAPALLY", "KOCHI", "TRIVANDRUM", "THRISSUR" };
		JComboBox s = new JComboBox(source);
		s.setBounds(50, 50, 100, 30);

		String[] dest = { "KOLLAM", "KAYANKULAM", "KOCHI", "KARUNAGAPALLY", "TRIVANDRUM", "THRISSUR" };
		...
		bb1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if (s.getItemAt(s.getSelectedIndex()) == d.getItemAt(d.getSelectedIndex())) {
					...
					String data1 = "The Route you entered is from " + s.getItemAt(s.getSelectedIndex()) + " to "
							+ d.getItemAt(d.getSelectedIndex());
		...
		conti.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (flag == true) {
					// JOptionPane.showMessageDialog(next,seats1.getItemAt(seats1.getSelectedIndex())+"
					// seats booked");
					Connection conn = null;
					Statement st = null;
					ResultSet rs = null;
					/*Class.forName("org.postgresql.Driver");
					Connection c = DriverManager.getConnection("jdbc:postgresql://localhost:5432/proj", "postgres", "Ravindra@1975");
				   ...
				   rs=st.executeQuery(sql);*/
					...
					try {
						...
						String sql = "INSERT INTO booktickets1 VALUES('" + s1 + "','" + s2 + "','" + s3 + "','" + s4
								+ "','" + s5 + "','" + s6 + "');";
						st = c.createStatement();
					...
					int input = JOptionPane.showOptionDialog(next,
							seats1.getItemAt(seats1.getSelectedIndex()) + " seat(s) booked", "The title",
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				try {
                    ...
                    c.setAutoCommit(false);
					String sql = "DELETE FROM booktickets1 WHERE username ='" + s1 + "';";
					String sql1 = "select * from Booktickets1 where username='" + s1 + "';";
					...
					if (s1.equals("")) {
						...
						int input = JOptionPane.showOptionDialog(next, "Ticket Cancelled", "The title",
								JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null, null, null);
...
class ShowBuses {
	public static void showBuses() {
		...
		JLabel sb2 = new JLabel(
				"BUS NUMBER                 BUS TYPE               TIME                AVAILABLE SEATS   ");
		sb2.setBounds(40, 30, 600, 18);
		c4.add(sb2);

		Object data[][] = { { "MH123", "AC", "11", "35" }, { "MH123", "AC", "11", "45" },
				{ "MH123", "AC", "11", "56" } };
